...
export function RoomGate() {
  ...
  return <RoomChoiceScreen onCreateRoom={() => setView("create")} />;
}

function RoomChoiceScreen({ onCreateRoom }: { onCreateRoom: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-zinc-950 text-white px-6">
      ...
      <div className="w-full max-w-sm flex flex-col gap-3">
        ...
        <button
          disabled
          className="w-full bg-zinc-800 text-zinc-400 font-medium rounded-lg py-3 disabled:opacity-40 disabled:cursor-not-allowed"
        >
          Join a Room
        </button>
      </div>
